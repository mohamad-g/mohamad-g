...
$strings = [
    'ar' => [
        ...
        'project_image_upload' => 'رفع صور المشاريع',
    ...
    'de' => [
        ...
        'project_image_upload' => 'Projektbilder hochladen',
...
// Get projects
try {
    $stmt = $pdo->prepare("SELECT id, title FROM projects WHERE user_id = ? AND language = ? ORDER BY title ASC");
...
if ($projectId) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM project_images WHERE project_id = ? ORDER BY id DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_project_image']) && isset($_FILES['project_image']) && $projectId) {
    $uploadDir = UPLOADS_DIR . '/projects';
    $result = uploadImage($_FILES['project_image'], $uploadDir);
    
    if ($result['success']) {
        try {
            // Insert project image
            $stmt = $pdo->prepare("INSERT INTO project_images (project_id, filename) VALUES (?, ?)");
            $stmt->execute([$projectId, $result['fileName']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_project_image']) && isset($_POST['image_id'])) {
    $imageId = (int)$_POST['image_id'];
    
    try {
        // Get image filename
        $stmt = $pdo->prepare("SELECT filename FROM project_images WHERE id = ?");
...
<head>
    ...
    <title><?php echo $s['project_images']; ?></title>
        ...
        <div class="row">
                ...
                <div class="position-sticky pt-3">
                    ...
                    <div class="dropdown px-3 mb-3">
                        ...
                        <ul class="dropdown-menu w-100" aria-labelledby="languageDropdown">
                            <li><a class="dropdown-item <?php echo $lang === 'ar' ? 'active' : ''; ?>" href="?lang=ar<?php echo $projectId ? '&project_id=' . $projectId : ''; ?>"><?php echo $s['arabic']; ?></a></li>
                            <li><a class="dropdown-item <?php echo $lang === 'de' ? 'active' : ''; ?>" href="?lang=de<?php echo $projectId ? '&project_id=' . $projectId : ''; ?>"><?php echo $s['german']; ?></a></li>
                        </ul>
                    </div>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><?php echo $s['project_images']; ?></h1>
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h5><?php echo $s['select_project']; ?></h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <select class="form-select" name="project_id" required>
                                        <option value=""><?php echo $s['select_project']; ?></option>
                                        <?php foreach ($projects as $project): ?>
                                        <option value="<?php echo $project['id']; ?>" <?php echo $projectId == $project['id'] ? 'selected' : ''; ?>><?php echo $project['title']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-check"></i> <?php echo $s['select_project']; ?>
                                    </button>
                                </div>
                            </div>
                        </form>
                        <?php else: ?>
                        <p><?php echo $s['no_projects']; ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <form method="POST" action="" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="mb-3">
                                        <label for="project_image" class="form-label"><?php echo $s['select_image']; ?></label>
                                        <input type="file" class="form-control" id="project_image" name="project_image" accept="image/jpeg, image/png, image/gif" required>
                                        <div class="form-text">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <button type="submit" name="upload_project_image" class="btn btn-primary d-block">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-lg-3 mb-4">
                                <div class="card h-100">
                                    <img src="<?php echo '../uploads/projects/' . $image['filename']; ?>" alt="Project Image" class="card-img-top project-image-preview">
                                    <div class="card-body text-center">
                                        <form method="POST" action="">
                                            <input type="hidden" name="project_id" value="<?php echo $projectId; ?>">
                                            <input type="hidden" name="image_id" value="<?php echo $image['id']; ?>">
                                            <button type="submit" name="delete_project_image" class="btn btn-danger btn-sm" onclick="return confirm('<?php echo $s['delete']; ?>?')">
